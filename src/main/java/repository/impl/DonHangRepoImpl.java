...
public class DonHangRepoImpl implements IDonHangRepository {
    ...
    private final RowMapper<DonHang> rowMapper = rs -> {
    ...
    public boolean add(DonHang entity) {
        ...
        try (Connection conn = DBConnect.getConnection()) {
            ...
            try (PreparedStatement psOrder = conn.prepareStatement(sqlOrder);
                ...
                if (entity.getChiTietDonHangList() != null) {
                    for (ChiTietDonHang item : entity.getChiTietDonHangList()) {
                        ...
                        try (ResultSet rsKeys = psDetail.getGeneratedKeys()) {
                            if (rsKeys.next()) {
                                long generatedId = rsKeys.getLong(1);
                                item.setMaCtdh(generatedId);
                                if (item.getToppingsList() != null) {
                                    for (ChiTietTopping tp : item.getToppingsList()) {
                                        psTopping.setLong(1, generatedId);
    ...
    public List<DonHang> getByKhachHang(String maKh) {
        ...
        return JdbcHelper.query(sql, rowMapper, maKh);
    }
    ...
    public boolean updateTrangThaiDon(String maDh, int status) {
        ...
        Timestamp ts = (status == 4) ? new Timestamp(System.currentTimeMillis()) : null;
        return JdbcHelper.update(sql, status, ts, maDh) > 0;
    }
    ...
    public boolean updateTrangThaiThanhToan(String maDh, int status) {
        ...
        return JdbcHelper.update(sql, status, maDh) > 0;
    }
    ...
    public List<ChiTietDonHang> getChiTietDonHang(String maDh) {
        String sql = "SELECT ct.*, sp.ten_sp, sz.ten_size " +
                ...
                "WHERE ct.ma_dh = ?";
        RowMapper<ChiTietDonHang> ctMapper = rs -> {
        ...
        return JdbcHelper.query(sql, ctMapper, maDh);
    }
    ...
    public List<ChiTietTopping> getToppingsOfChiTiet(long maCtdh) {
        String sql = "SELECT ct.*, t.ten_tp " +
                ...
                "WHERE ct.ma_ctdh = ?";
        RowMapper<ChiTietTopping> tpMapper = rs -> {
            ChiTietTopping tp = new ChiTietTopping();
            tp.setMaCtdh(rs.getLong("ma_ctdh"));
            tp.setMaTp(rs.getString("ma_tp"));
            tp.setSoLuong(rs.getInt("so_luong"));
            tp.setGiaChotTp(rs.getInt("gia_chot_tp"));
            tp.setTenTopping(rs.getString("ten_tp"));
            return tp;
        };
        return JdbcHelper.query(sql, tpMapper, maCtdh);
    }
    ...
    public String generateNextMaDh() {
        ...
        String sql = "SELECT COALESCE(MAX(CAST(RIGHT(ma_dh, 5) AS INT)), 0) + 1 FROM DON_HANG WHERE ma_dh LIKE ?";
        Integer nextVal = JdbcHelper.queryForObject(sql, rs -> rs.getInt(1), prefix + "%");
        int finalVal = (nextVal != null) ? nextVal : 1;
        return prefix + String.format("%05d", finalVal);
    }
}
