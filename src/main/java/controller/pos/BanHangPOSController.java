...
public class BanHangPOSController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (uri.endsWith("/pos/search-customer")) {
            ...
            return;
        } else if (uri.endsWith("/pos/bill-detail")) {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        } else if (uri.endsWith("/pos/search-customer")) {
            performSearchCustomer(request, response);
        } else if (uri.endsWith("/pos/bill-detail")) {
            performGetBillDetail(request, response);
        } else {
    ...
    private void performApplyVoucher(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        int tongTienHang = WebUtil.getIntParameter(request, "tongTienHang", 0);
    ...
    private void performGetBillDetail(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (dh != null) {
            ...
            String textKh = "Khách lẻ vãng lai";
            String sdtKh = "N/A";
            if (dh.getMaKh() != null) {
            ...
            json.append("\"tenKhachHang\":\"").append(textKh).append("\",");
    ...
    private void performCheckout(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            String maKh = request.getParameter("maKh");
            int loaiDonHang = WebUtil.getIntParameter(request, "loaiDonHang", 1);
            int maPt = WebUtil.getIntParameter(request, "maPt", 1);
            String maKm = request.getParameter("maKm");
            int tongTienHang = WebUtil.getIntParameter(request, "tongTienHang", 0);
            int tienGiamGia = WebUtil.getIntParameter(request, "tienGiamGia", 0);
            int diemSuDung = WebUtil.getIntParameter(request, "diemSuDung", 0);
            int tienTruDiem = WebUtil.getIntParameter(request, "tienTruDiem", 0);
            int tongPhaiTra = WebUtil.getIntParameter(request, "tongPhaiTra", 0);
            ...
            if (itemMaSp == null) {
                itemMaSp = request.getParameterValues("item_maSp");
            }
            String[] itemMaSize = request.getParameterValues("item_maSize[]");
            if (itemMaSize == null) {
                itemMaSize = request.getParameterValues("item_maSize");
            }
            String[] itemSoLuong = request.getParameterValues("item_soLuong[]");
            if (itemSoLuong == null) {
                itemSoLuong = request.getParameterValues("item_soLuong");
            }
            String[] itemGiaChot = request.getParameterValues("item_giaChot[]");
            if (itemGiaChot == null) {
                itemGiaChot = request.getParameterValues("item_giaChot");
            }
            String[] itemMucDa = request.getParameterValues("item_mucDa[]");
            if (itemMucDa == null) {
                itemMucDa = request.getParameterValues("item_mucDa");
            }
            String[] itemMucDuong = request.getParameterValues("item_mucDuong[]");
            if (itemMucDuong == null) {
                itemMucDuong = request.getParameterValues("item_mucDuong");
            }
            String[] itemGhiChuMon = request.getParameterValues("item_ghiChuMon[]");
            if (itemGhiChuMon == null) {
                itemGhiChuMon = request.getParameterValues("item_ghiChuMon");
            }
            String[] itemToppingKeys = request.getParameterValues("item_toppingKeys[]");
            if (itemToppingKeys == null) {
                itemToppingKeys = request.getParameterValues("item_toppingKeys");
            }
            List<ChiTietDonHang> items = new ArrayList<>();
            if (itemMaSp != null) {
                for (int i = 0; i < itemMaSp.length; i++) {
                    if (itemMaSp[i] == null || itemMaSp[i].trim().isEmpty()) {
                        continue;
                    }
                    ...
                    ctdh.setGiaChot(priceChot);
                    ctdh.setMucDa(itemMucDa != null && i < itemMucDa.length ? itemMucDa[i] : "100%");
                    ctdh.setMucDuong(itemMucDuong != null && i < itemMucDuong.length ? itemMucDuong[i] : "100%");
    ...
    private void performChangePassword(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        // FIX: So khớp & đổi mật khẩu băm kèm muối tên đăng nhập của nhân sự
        String hashedOld = SecurityUtil.hashWithSalt(oldPassword, currentStaff.getTenDangNhap());
        if (!currentStaff.getMatKhau().equals(hashedOld)) {
            response.getWriter().write("{\"status\":\"ERROR\",\"message\":\"Mật khẩu cũ không chính xác!\"}");
            return;
        }
        currentStaff.setMatKhau(SecurityUtil.hashWithSalt(newPassword, currentStaff.getTenDangNhap()));
        boolean success = nhanVienService.updateNhanVien(currentStaff);
        if (success) {
            session.setAttribute("user", currentStaff);
            response.getWriter().write("{\"status\":\"SUCCESS\"}");
        } else {
            response.getWriter().write("{\"status\":\"ERROR\",\"message\":\"Cập nhật mật khẩu thất bại!\"}");
        }
    }
}
