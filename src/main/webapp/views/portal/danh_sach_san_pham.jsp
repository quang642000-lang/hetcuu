...
<div class="container py-4">
    <div class="row g-4">
            ...
            <div class="card border-0 p-4 shadow-sm" style="border-radius: 16px;">
                ...
                <form action="${pageContext.request.contextPath}/products" method="GET" class="mb-4">
                    <div class="input-group">
                        <input type="text" name="keyword" class="form-control" placeholder="Tìm tên món..." value="<c:out value="${searchKeyword}"/>">
        ...
        <div class="col-12 col-lg-9">
            <!-- THANH ĐIỀU HƯỚNG TÌM KIẾM NHANH -->
            <div class="card border-0 p-3 mb-4 shadow-sm d-flex flex-row justify-content-between align-items-center" style="border-radius: 12px;">
                <span class="text-muted fw-medium">Tìm thấy <strong class="text-dark">${products.size()}</strong> cốc nước thơm ngon</span>
                <div class="btn-group">
                    <button class="btn btn-sm btn-light border dropdown-toggle" type="button" data-bs-toggle="dropdown">Sắp xếp theo</button>
                    <ul class="dropdown-menu">
                        <li><a class="dropdown-menu dropdown-item small" href="#">Giá bán: Thấp đến Cao</a></li>
                        <li><a class="dropdown-menu dropdown-item small" href="#">Giá bán: Cao đến Thấp</a></li>
                    </ul>
                </div>
            </div>
                    ...
                    <c:when test="${not empty products}">
                        <c:forEach var="item" items="${products}">
                            <div class="col-12 col-md-6 col-lg-4">
                                <div class="card product-card h-100 border-0 shadow-sm bg-white p-3">
                                    <div class="position-relative mb-3">
                                        <c:if var="newTag" test="${item.isNew}">
                                            <span class="position-absolute top-0 start-0 badge bg-warning text-dark fw-bold m-2 px-2.5 py-1.5" style="border-radius: 8px;">✨ MỚI</span>
                                        </c:if>
                                        <c:if var="hotTag" test="${item.isBestseller}">
                                            <span class="position-absolute top-0 end-0 badge bg-danger text-white fw-bold m-2 px-2.5 py-1.5" style="border-radius: 8px;">HOT 🔥</span>
                                        </c:if>
                                    ...
                                    <div class="d-flex justify-content-between align-items-center mt-auto">
                                        ...
                                        <a href="${pageContext.request.contextPath}/product/detail?id=${item.maSp}" class="btn btn-outline-success btn-sm px-3 fw-bold rounded-pill">
                                            Chi tiết
                                        </a>
                                    </div>
                                </div>
