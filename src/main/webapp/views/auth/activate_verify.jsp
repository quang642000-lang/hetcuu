...
<head>
  ...
  <style>
    ...
    .otp-card { width: 100%; max-width: 480px; background-color: #ffffff; border-radius: 16px; box-shadow: 0 10px 30px rgba(16, 185, 129, 0.1); }
    .otp-digit-input { width: 44px; height: 50px; font-size: 24px; font-weight: bold; text-align: center; border: 2px solid #cbd5e1; border-radius: 8px; margin: 0 4px; outline: none; transition: all 0.15s ease; }
    .otp-digit-input:focus { border-color: #10b981; box-shadow: 0 0 0 3px rgba(16, 185, 129, 0.15); }
  </style>
</head>
<body>
<div class="otp-card p-4 p-md-5">
  <div class="text-center mb-4">
    <div class="text-success fs-1 mb-2"><i class="bi bi-shield-fill-check"></i></div>
    <h4 class="fw-bold mb-1">XÁC MINH OTP KÍCH HOẠT</h4>
    <p class="text-muted small">Chúng tôi đã gửi mã xác thực 6 chữ số vào hòm thư Email liên kết của bạn. Hãy kiểm tra hộp thư đến (hoặc hòm thư Spam) và nhập vào đây.</p>
  </div>
  ...
  <form action="${pageContext.request.contextPath}/activate/verify" method="POST" id="otpForm">
    ...
    <button type="submit" class="btn btn-primary-teapos w-100 py-2.5 fw-bold text-uppercase"><i class="bi bi-shield-lock-fill me-1"></i> Xác minh kích hoạt</button>
  </form>
</div>
<script>
